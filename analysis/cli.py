...
def analyze_no_model(args: argparse.Namespace) -> Path:
    ...
    try:
        ...
        motion_times, motion, camera_stability = motion_signal(proxy_path, court, args.analysis_fps)
        period = 1 / args.analysis_fps
        audio_times, audio = audio_signal(proxy_path, proxy_info["duration"], period, proxy_info["hasAudio"])
        ...
        if court.source == "fallback-region":
            warnings.append("Court lines were inconclusive; motion used a conservative fallback region.")
