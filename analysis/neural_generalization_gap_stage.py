"""Bounded source-gap staging amendment; original registered stager is unchanged.
...
def pts_selection(pts, duration, frequency):
    ...
    require(pts.ndim == 1 and len(pts) and np.isfinite(pts).all()
            and np.all(np.diff(pts) > 0), 'Invalid native PTS inventory')
    times = np.arange(0., duration, 1./frequency, dtype=np.float64)
    ...
    require(len(times) and np.max(abs(pts[selected]-times)) <= .25+1e-9,
            'Source gap exceeds the amended250ms nearest-frame tolerance')
...
def stage_record(plan_path, recording_id, *, root=ROOT):
    ...
    amendment_path = root/'timing-amendment-v1/protocol.json'
    amendment = read(amendment_path)
    require(amendment['basePlan'] == identity(plan_path) and amendment['maximumNearestErrorSeconds'] == .25,
            'Source timing amendment differs')
    for reference in amendment['code'].values():
        verified(reference)
    for reference in amendment['diagnostics']:
        verified(reference)
    require(recording_id in amendment['affectedRecordings'] or recording_id == plan['engineeringRecord'],
            'Only registered timing failures and the engineering control may use this stager')
    require(source['environment'] != 'beach', 'Beach is outside this plan')
    folder = root/'staged-timing-v1'/recording_id
    receipt_path = folder/'receipt.json'
    lineage = {'plan': identity(plan_path), 'source': source}
    if receipt_path.exists():
        receipt = read(receipt_path)
        require(receipt['lineage'] == lineage and receipt['samplingAmendment'] == identity(amendment_path), 'Amended staging resume differs')
    ...
    if sampler == 'media-pts':
        ...
        require(np.max(abs(pts[ordinals]-times)) <= .25+1e-9, '4Hz source-gap selection exceeds amended250ms bound')
    ...
    try:
        while capture.grab():
            ...
            while cursor < len(ordinals) and ordinal == int(ordinals[cursor]):
    ...
    errors = np.asarray(actual_pts, np.float64)-times
    bad = np.flatnonzero(abs(errors) > .125+1e-9)
    duplicates = np.flatnonzero(np.diff(ordinals) == 0)+1
    gap_info = {'maximumNearestErrorSeconds': float(abs(errors).max()),
        'affectedTicks': [{'tickIndex': int(i), 'time': float(times[i]), 'selectedPts': float(actual_pts[i]),
                           'ordinal': int(ordinals[i]), 'errorSeconds': float(errors[i])} for i in bad],
        'duplicateOrdinalTickIndexes': duplicates.tolist(),
        'mobileTickSelection': 'Even4Hz tick indexes at2Hz, with their actual selected PTS retained in quality.',
        'missingnessCaveat': 'A real camera gap supplies no new pixels; nearest recorded frames may repeat. No interpolation or synthetic motion; inference remains valid and no gold ignored intervals are created.'}
    receipt = {'lineage': lineage, 'id': recording_id, 'sourceGroup': source['sourceGroup'],
        'samplingAmendment': identity(amendment_path), 'sourceGapDiagnostics': gap_info,
