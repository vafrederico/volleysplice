...
class RallyDetectionTests(unittest.TestCase):
    def test_detects_and_pads_one_sustained_activity_interval(self):
        ...
        self.assertAlmostEqual(rallies[0]["end"], 10.0)

    def test_closes_a_short_internal_gap(self):
        rallies = detect_rallies(timeline((5, 8), (8.5, 12)), 24, camera_stability=0.95)
        self.assertEqual(len(rallies), 1)
        self.assertLessEqual(rallies[0]["start"], 5)
        self.assertGreaterEqual(rallies[0]["end"], 12)

    def test_does_not_merge_activity_across_one_second_of_dead_time(self):
    ...
    def test_detection_settings_serialize_with_analysis_field_names(self):
        self.assertEqual(DetectionSettings().as_dict()["maxGapSeconds"], 0.75)
        self.assertEqual(DetectionSettings().as_dict()["endingTailSeconds"], 0.0)
