...
class SideSwitchFeatureDevelopmentTest(unittest.TestCase):
    ...
    def test_t2_profile_appends_only_conditional_transport_core(self):
        ...
        self.assertEqual(
            T2_SWAP_ONLY_PROFILE.feature_names,
            (*BASE_FEATURE_NAMES, T2_CORE_FEATURE_NAMES[0]),
        )
        self.assertEqual(
            T2_CONDITIONAL_ONLY_PROFILE.feature_names,
            (*BASE_FEATURE_NAMES, T2_CORE_FEATURE_NAMES[1]),
        )

    def test_t4_profile_appends_only_selective_far_jersey_core(self):
        from analysis.side_switch_t4_selective_far import T4_CORE_FEATURE_NAMES

        self.assertEqual(
            T4_PROFILE.feature_names, (*BASE_FEATURE_NAMES, *T4_CORE_FEATURE_NAMES)
        )
    ...
    def test_t14_profile_appends_only_dominant_tracklet_core(self):
        from analysis.side_switch_t14_dominant_tracklet_medoid import (
            T14_CORE_FEATURE_NAMES,
        )

        self.assertEqual(
            T14_PROFILE.feature_names, (*BASE_FEATURE_NAMES, *T14_CORE_FEATURE_NAMES)
        )
        self.assertEqual(len(T14_PROFILE.feature_names), 37)
        self.assertNotIn(
    ...
    def test_t19_profile_appends_only_cross_representation_core(self):
        from analysis.side_switch_t19_cross_representation import T19_CORE_FEATURE_NAMES

        self.assertEqual(
            T19_PROFILE.feature_names, (*BASE_FEATURE_NAMES, *T19_CORE_FEATURE_NAMES)
        )
        self.assertEqual(len(T19_PROFILE.feature_names), 36)

    def test_t20_profile_appends_only_compact_candidate_core(self):
        from analysis.side_switch_t20_compact_candidate import T20_CORE_FEATURE_NAMES

        self.assertEqual(
            T20_PROFILE.feature_names, (*BASE_FEATURE_NAMES, *T20_CORE_FEATURE_NAMES)
        )
        self.assertEqual(len(T20_PROFILE.feature_names), 36)
