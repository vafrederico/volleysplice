const GLUE_URL = "/on-device/libswresample/libswresample.mjs";
const WASM_URL = "/on-device/libswresample/libswresample.wasm";
...
async function loadModule(): Promise<LibswresampleModule> {
  if (!assetsPromise) {
    assetsPromise = (async () => {
      ...
      const imported = (await import(/* webpackIgnore: true */ GLUE_URL)) as {
