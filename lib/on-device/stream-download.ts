...
function waitForActivation(registration: ServiceWorkerRegistration): Promise<ServiceWorker> {
  ...
  if (!candidate) return Promise.reject(new Error("The export service worker did not install."));

  return new Promise((resolve, reject) => {
    const timeout = window.setTimeout(() => {
      candidate.removeEventListener("statechange", onStateChange);
      reject(new Error("The export service worker did not become active in time."));
    }, 15_000);
    const onStateChange = () => {
      if (candidate.state === "activated") {
        ...
        window.clearTimeout(timeout);
        candidate.removeEventListener("statechange", onStateChange);
        reject(new Error("The export service worker installation was replaced."));
      }
    };
...
export function prepareServiceWorkerStreamDownload(
  ...
  registrationPromise = (async () => {
    ...
    try {
      ...
      const registration = await navigator.serviceWorker.register(absoluteScriptUrl, {
...
export function startServiceWorkerStreamDownload(fileName: string): StreamDownload {
  const ready = readyRegistration;
  if (!ready?.worker || ready.worker.state !== "activated") {
    throw new Error("Experimental direct download is not ready. Use compatible export and retry.");
  }
  ...
  channel.port1.onmessage = (event: MessageEvent) => {
    const message = event.data as { protocol?: string; type?: string; reason?: string };
    ...
    if (message.type === "pull") {
      ...
      fail(new Error(message.reason || "The browser canceled the streamed download."));
    }
  };
  channel.port1.start();
  ...
  const writable = new WritableStream<Uint8Array>({
    async write(chunk) {
      ...
      channel.port1.postMessage(
        { protocol: PROTOCOL, type: "chunk", buffer },
        [buffer],
      );
    },
    close() {
    ...
    abort(reason) {
      channel.port1.postMessage({
        protocol: PROTOCOL,
        type: "error",
        reason: reason instanceof Error ? reason.message : String(reason ?? "Export canceled"),
      });
      channel.port1.close();
    },
  });
  ...
  const downloadUrl = new URL(`__volleycut_export_download__/${token}`, ready.registration.scope);
