...
function UtilityLinks({ state, quiet = false }: { state: Prototype; quiet?: boolean }) {
  return (
    <nav className={`td-utility ${quiet ? "is-quiet" : ""}`} aria-label="Product links and downloads">
      ...
      <a href="/android/VolleyCut-v0.10.9-arm64-release-signed.apk" download>Android app</a>
  ...
  return (
    <div className="td-help-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) state.setHelpOpen(false); }}>
      <aside ref={dialogRef} className="td-help" role="dialog" aria-modal="true" aria-labelledby="td-help-title" aria-describedby="td-help-description" tabIndex={-1}>
        ...
        <div className="td-help-links"><a href="/privacy.html">Read privacy policy</a><a href="/terms.html">Read terms</a><a href="/android/VolleyCut-v0.10.9-arm64-release-signed.apk" download>Download Android app</a></div>
      </aside>
    </div>
  );
}
...
function FilmRoom({ review }: { review: ReadyDesignReview }) {
  ...
  return (
    <main className="taste-root taste-film-room" data-theme={state.dark ? "dark" : "light"}>
      <header className="fr-header"><Brand /><StageButtons state={state} /><div className="fr-actions"><ProjectChip state={state} /><a href="/privacy.html">Privacy</a><a href="/terms.html">Terms</a><a href="/android/VolleyCut-v0.10.9-arm64-release-signed.apk" download>Android</a><button type="button" onClick={() => state.setDark(!state.dark)}>{state.dark ? "Light theme" : "Dark theme"}</button></div></header>
      {state.stage === "review" ? (
