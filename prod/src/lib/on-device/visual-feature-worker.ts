...
async function loadWorkerOpenCv(): Promise<CvRuntime> {
  runtimePromise ??= (async () => {
    if (!openCvUrl) throw new Error("The OpenCV worker asset URL was not configured.");
    await import(/* @vite-ignore */ openCvUrl);
    ...
    try {
      if (request.type === "initialize") {
        ...
        wasmReducer = request.reductionKernel === "wasm"
          ? await WasmVisualFeatureReducer.load(reductionWasmUrl)
          : null;
        post({
          type: "ready",
          openCvLoadMs,
