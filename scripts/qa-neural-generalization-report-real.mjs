...
// Read-only QA of the finished audited report; no fixtures are injected into it.
...
export async function run(args){
 ...
 assert.equal(render.kind,'matched-variant-report-view-v3');assert.equal(render.sourceSha256,source.sha256);
 if(!selfTest||render.output)assert.equal(render.output.sha256,html.sha256);
 const renderer=await identity(fileURLToPath(new URL('./render-neural-generalization-report-v3.py',import.meta.url)));
 assert.equal(renderer.sha256,'87fafb74c793051b93cb1a0000cf2fd21464f88f7ec1ec196fcde9468af58dde');assert.equal(render.rendererSha256,renderer.sha256);
 let audit=null;
 ...
 const verify=async(label)=>{
  ...
  const result={label,state:current,rows:expected.length,complete:expected.filter(complete).length,tableRows:table.length,dimensions,memory:await memory()};checks.views.push(result);return result;
 };
 try{
  ...
  const result={kind:'source-derived-generalization-report-ui-qa-v1',passed:true,syntheticSelfTest:selfTest,actualAuditedReport:!selfTest,source,html,renderReceipt:renderIdentity,renderer,audit,script:await identity(fileURLToPath(import.meta.url)),browser:await browser.version(),checks,errors,externalRequests:requests,profileAndTempRoot:process.env.TEMP};
