...
// Read-only v4 QA, derived explicitly from frozen v3 QA 9c29eae155d2494e69f2b3f5603560877e5e688720f30d75ba2269cc51e17d79.
// Original source is preserved; only renderer identity/version and scoped task checks are added.
...
export async function run(args){
 ...
 assert.equal(render.kind,'scoped-task-report-view-v4');assert.equal(render.sourceSha256,source.sha256);
 if(!selfTest||render.output)assert.equal(render.output.sha256,html.sha256);
 const renderer=await identity(fileURLToPath(new URL('./render-neural-generalization-report-v4.py',import.meta.url)));
 assert.equal(renderer.sha256,'31c9acee117ef58ae3b736b138cc95f2dd3ac96d85aedd2a748616bc8956a0cf');assert.equal(render.rendererSha256,renderer.sha256);
 let audit=null;
 ...
 try{
  ...
  assert.equal(await page.evaluate(()=>window.__reportQA.presentationVersion),4);
  ...
  const result={kind:'source-derived-generalization-report-ui-qa-v4',passed:true,syntheticSelfTest:selfTest,actualAuditedReport:!selfTest,source,html,renderReceipt:renderIdentity,renderer,audit,script:await identity(fileURLToPath(import.meta.url)),browser:await browser.version(),checks,errors,externalRequests:requests,profileAndTempRoot:process.env.TEMP};
