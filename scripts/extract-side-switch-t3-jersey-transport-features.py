#!/usr/bin/env python3
"""Extract immutable label-free team-isolated jersey transport features for T3."""
...
DEFAULT_OUTPUT = (
    REPORTS / "side-switch-t3-team-isolated-jersey-transport-features-v1.json"
)
EXPECTED_SHA256 = {
...
def extract(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if output.exists():
        raise FileExistsError(f"refusing to overwrite T3 feature artifact: {output}")
    hashes = {name: _sha256(path) for name, path in paths.items()}
    if args.enforce_source_hash and hashes != EXPECTED_SHA256:
        raise ValueError(f"T3 source identity changed: {hashes}")
    ...
    if source["scope"] != visual["scope"]:
        raise ValueError("T3 source and visual-summary scopes differ")
    ...
    if [str(row["eventId"]) for row in source_rows] != [
        ...
        raise ValueError("T3 source row identity/order differs from Visual Summary V2")
    if len(source_rows) != EXPECTED_ROWS:
        raise ValueError("T3 source row count changed")
    ...
    for recording_number, recording_id in enumerate(recording_ids, start=1):
        ...
        if (
            ...
            raise ValueError(f"T3 video provenance changed for {recording_id}")
        ...
        if not capture.isOpened():
            raise RuntimeError(f"could not open T3 source video: {video_path}")
        ...
        extraction_audit[recording_id] = {
            ...
            "detectorTileCalls": len(windows) * len(ENDPOINT_FRAME_FRACTIONS) * 4,
            "frameErrors": 0,
    ...
    if len(endpoint_summaries) != EXPECTED_ENDPOINTS:
        raise ValueError("T3 unique endpoint count changed")
    ...
    for row in source_rows:
        ...
        if str(row["kind"]) == "adjacent-rally-boundary":
            ...
            values, diagnostics = team_transport_features(before, after)
            updated["features"].update(values)
            updated["t3JerseyTransport"] = {
                "status": "ok",
    ...
    for before, after in zip(source_rows, output_rows, strict=True):
        for name, value in before["features"].items():
            if float(after["features"][name]) != float(value):
                raise ValueError(f"T3 changed existing feature {name}")
    ...
    four_team = float(
        np.mean(
            [
                row["features"]["jerseyTeamReliabilityMinimum"] > 0.0
                and row["features"]["jerseyCrossSideSimilarityMinimum"] > 0.0
    ...
    reliable_swap_nonzero = float(
        np.mean(
            [row["features"]["jerseyReliableSwapEvidence"] > 0.0 for row in boundary_rows]
        )
    )
    core_nonconstant = {
        name: float(np.ptp([row["features"][name] for row in boundary_rows])) > 1e-15
        for name in T3_CORE_FEATURE_NAMES
    }
    existing_names = tuple(BASE_FEATURE_NAMES)
    if len(existing_names) != 34:
        raise ValueError("T3 expected the exact existing 34-input signature")
    correlations: list[dict[str, Any]] = []
    for index, left_name in enumerate(T3_CORE_FEATURE_NAMES):
        left_values = np.asarray([row["features"][left_name] for row in boundary_rows])
        compare_names = (*T3_CORE_FEATURE_NAMES[index + 1 :], *existing_names)
    ...
    core_correlations = [
        ...
        if value["left"] in T3_CORE_FEATURE_NAMES
        and value["right"] in T3_CORE_FEATURE_NAMES
    ]
    ...
    performance = {
        ...
        "frameRequests": len(endpoint_summaries) * len(ENDPOINT_FRAME_FRACTIONS),
        "detectorTileCalls": len(endpoint_summaries)
    ...
    if (
        ...
        raise ValueError("T3 frame/detector budget changed")
    checks.update(
        {
    ...
    payload = {
        "schemaVersion": 1,
        "kind": "volleycut-side-switch-t3-team-isolated-jersey-transport-features-v1",
        ...
        "contract": {
            ...
            "featureNames": list(T3_FEATURE_NAMES),
            "futureFirstHeadFeatureNames": list(T3_CORE_FEATURE_NAMES),
            "endpointFrameFractions": list(ENDPOINT_FRAME_FRACTIONS),
            "candidateGeneration": "unchanged",
            "labelUse": "none; no audit, feedback, label, or T2 model-result artifact loaded",
        ...
        "limitations": [
            ...
            "Anonymous jersey descriptors are not supervised team identities.",
            "Browser and Android latency has not been measured.",
        ],
    }
