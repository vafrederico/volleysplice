#!/usr/bin/env python3
"""Schedule reviewed_export/linear inner fits for seed20260918 or1729 only.

The three-fit pilot and nine-fit continuation call the unchanged fit_model on
canonical paths. No selection, refits, result aggregation or bound-code edits.
"""
...
COHORT, KIND, SEED = "reviewed_export", "linear", 20260918
...
def main_progress_markers(study):
    # Seed3407 is owned solely by main. Never use a precomputed seed directory
    # as a main progress marker: worker1729 would otherwise trip its own guard.
    paths = [study / "report.json", study / "fits" / COHORT / KIND / "3407"]
...
def main():
    ...
    parser.add_argument("--fit-count", type=int, default=3)
    parser.add_argument("--seed", type=int, choices=(20260918, 1729), default=20260918)
    ...
    write_new(args.record_output / "execution.json", {
        "kind": "frozen-expanded-linear-inner-fit-scheduling-v1", "createdAt": datetime.now(timezone.utc).isoformat(),
