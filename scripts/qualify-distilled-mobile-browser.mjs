...
// Derived from the pinned recognition harness; actual distilled encoder + head.
...
const originalHarnessPath = join(fileURLToPath(new URL(".", import.meta.url)), "qualify-neural-recognition-browser.mjs");
const originalHarnessSha256 = hash(await readFile(originalHarnessPath));
if (originalHarnessSha256 !== "918978de78d58355d59f1ccd43c7256fd659c89a071feb5ddbefbd9d37606c6a") throw new Error("Pinned browser helper changed");
const fixturePath = join(source, "fixture-manifest.json");
const manifestBytes = await readFile(fixturePath);
const manifest = JSON.parse(manifestBytes);
if (manifest.schemaVersion !== 1 || manifest.kind !== "distilled-mobile-portability-fixtures-v1" || manifest.temporalModelsTrained !== true || manifest.encoderTrained !== true) throw new Error("Unexpected trained student fixture contract");
if (manifest.models.length !== 2 || manifest.models[0].role !== "image-encoder" || manifest.models[1].role !== "temporal-head") throw new Error("Expected one encoder and one temporal graph");
for (const model of manifest.models) {
  if (model.checkpoint?.seed !== 3407 || model.checkpoint?.fit !== "inner-0-1" || model.checkpoint?.temporalEpoch !== 60) throw new Error("Fixed checkpoint identity differs");
  if (model.role === "temporal-head" && (model.state !== "trained completed checkpoint with fitted scaler" || model.checkpoint.scalerIncludedInGraph !== true)) throw new Error("Temporal checkpoint/scaler missing");
  if (model.role === "image-encoder" && (model.state !== "trained distilled encoder with regional pooling" || model.checkpoint.scalerIncludedInGraph !== false || model.classifierAndDistillationProjectionOmitted !== true)) throw new Error("Trained encoder identity differs");
}
  ...
  result = await page.evaluate(async ({ manifest, repeats }) => {
    ...
    for (const model of manifest.models) {
      ...
      try {
        ...
        models.push({ name: model.name, role: model.role, state: model.state, checkpoint: model.checkpoint, graph: model.graph, cases, chunkEquivalence,
...
const report = { schemaVersion: 1, createdAt: new Date().toISOString(), ...result,
  scope: "Isolated desktop Chrome WASM CPU engineering parity; actual distilled encoder and fixed trained temporal checkpoint/scaler",
  ...
  scriptSha256: hash(await readFile(fileURLToPath(import.meta.url))), originalHarnessSha256,
  limitations: ["No physical phone or Android delegate measurement; browser is desktop Chrome.",
    ...
    "The encoder and head are separate prepared-input graphs. Float16 token rounding, temporal alignment, quality features and AV calculation are not qualified end to end.",
