...
def audit_historical(entry, evidence, inventory, metrics):
    ...
    if report.get('kind') == 'audited-historical-precision-sweep-projection-v1':
        ...
        for p in path.parent.glob('audit*.json'):
            doc = io.read(p)
            if doc.get('passed') is True and doc.get('report') == entry['result']:
                io.require(doc['kind'] == 'independent-recall-floor-sweep-audit-v1'
                           and doc['auditor'] == io.identity(REPO/'scripts/audit-neural-recall-sweep.py'), 'Historical numerical auditor differs')
        ...
        for panel_id, panel in cell['panels'].items():
            ...
            for floor in panel['result']['floors']:
                for raw in floor['predictions']:
                    ...
                    compare(raw['durationSeconds'], authoritative['durationSeconds'], 'Historical duration')
...
def audit(index_path, report_path, output, node):
    ...
    result = {'kind': 'independent-generalization-result-report-audit-v1', 'passed': True,
