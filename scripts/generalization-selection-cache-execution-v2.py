...
def validate_dispatch():
    """Inspect actual frozen callables; do not invoke any numerical work."""
    import inspect
    targets = [('select', body.old.select_task_with_correction, 5),
        ('ordinary', duration.audit_selection, 3), ('correction', correction.audit, 3),
        ('freeze-selections', body.freeze_selections, 1), ('evaluate', body.evaluate, 1),
        ('audit-results', body.audit_results, 1), ('global-gate', body.global_gate, 1),
        ('publication-preflight', body.publication_preflight, 2)]
    result = []
    for name, function, count in targets:
        inspect.signature(function).bind(*[None]*count)
        result.append({'action': name, 'callable': function.__name__, 'signature': str(inspect.signature(function)),
            'source': io.identity(inspect.getsourcefile(function)), 'positionalArgumentCount': count})
    return result
...
def verify_plan(path):
    ...
    io.require(plan['code']['scripts/generalization-selection-cache-execution-v2.py'] == io.identity(__file__), 'Cache workflow changed')
    for key in ('supersedes', 'preservedFailure'): verified(plan[key])
    io.require(plan['dryDispatchChecks'] == validate_dispatch(), 'Frozen delegated callable signature/source differs')
...
def register(args):
    ...
    prior = io.read(verified(io.identity(args.supersedes)))
    failure = io.read(verified(io.identity(args.failure)))
    io.require(prior['policy'] == POLICY and failure['plan'] == io.identity(args.supersedes)
        and failure['noNumericalStageExecuted'] is True and failure['qualificationOutputAbsent'] is True
        and not Path(prior['qualification']['output']).exists(), 'Preserved dispatch failure differs')
    dispatch_checks = validate_dispatch()
    ...
    names = ('analysis/neural_selection_identity_cache.py', 'scripts/generalization-selection-cache-execution-v2.py',
        'scripts/audit-neural-generalization-numerics.py', 'scripts/audit-neural-historical-refits.py',
        'scripts/audit-neural-selection-identity-cache-v2.py', 'analysis/tests/test_selection_identity_cache.py',
        'analysis/tests/test_selection_identity_cache_audit.py', 'analysis/tests/test_selection_identity_cache_v2.py',
        'analysis/tests/test_selection_identity_cache_audit_v2.py')
    io.require(tasks == prior['tasks'], 'V2 task/stage/unchanged control inventory differs')
    io.write_new(args.output, {'kind': 'generalization-selection-identity-cache-amendment-v1', 'policy': POLICY,
        'createdAtUTC': now(), 'supersedes': io.identity(args.supersedes), 'preservedFailure': io.identity(args.failure),
        'dryDispatchChecks': dispatch_checks, 'durationPlan': io.identity(args.duration_plan), 'correctionPlan': inherited['correctionPlan'],
        ...
        'code': {**prior['code'], **inherited['code'], **{name: io.identity(REPO/name) for name in names}},
        ...
        'mandatoryPublicationWorkflow': 'scripts/generalization-selection-cache-execution-v2.py'})
...
def execute_stage(plan_path, plan, cache, job, stage, output, receipt_path):
    ...
    with installed(cache):
        ...
        if stage in ('select', 'qualification-select'):
            body.old.select_task_with_correction(Path(plan['correctionPlan']['path']), Path(job['task']['path']),
                folder, Path(output), Path(plan['historicalProtocol']['path']).parent)
...
def cold_gate(path):
    ...
    io.require(gate['kind'] == 'independent-selection-identity-cache-cold-audit-v1' and gate['passed'] is True
        ...
        and gate['auditor'] == io.identity(REPO/'scripts/audit-neural-selection-identity-cache-v2.py'), 'Complete independent cold cache gate required')
...
def main():
    ...
    for key in ('duration-plan', 'registration', 'protocol', 'supersedes', 'failure', 'output'): reg.add_argument('--'+key, type=Path, required=True)
