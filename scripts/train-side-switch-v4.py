#!/usr/bin/env python3
"""Freeze and evaluate the multi-frame, court-normalized v4 specialist."""
...
DEFAULT_MODEL_DIR = (
    ROOT / "models/side-switch-specialist-v4-multiframe-normalized"
)
DEFAULT_EVALUATION = (
    ROOT
...
def _load_features(path: Path, enforce_source_hash: bool) -> Mapping[str, Any]:
    if enforce_source_hash and _sha256(path) != EXPECTED_FEATURE_SHA256:
        raise ValueError("v4 feature artifact identity changed")
    payload = _load(path)
    if payload.get("kind") != FEATURE_ARTIFACT_KIND:
        raise ValueError("feature artifact is not side-switch v4")
    if payload.get("frozenRecordingSplit") != {
        role: list(recording_ids)
        for role, recording_ids in FROZEN_RECORDING_SPLIT.items()
    }:
        raise ValueError("v4 feature artifact does not match the frozen split")
    return payload
...
def _events(payload: Mapping[str, Any], roles: set[str]) -> list[V3Event]:
    ...
    if not isinstance(rows, list):
        raise ValueError("v4 feature artifact has no rows")
    events = [
        V3Event(
    ...
    if not events:
        raise ValueError(f"v4 feature artifact has no events for roles {sorted(roles)}")
    if any(event.label not in {0, 1} for event in events):
        raise ValueError("v4 event labels must be binary")
    return events
...
def freeze(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if model_dir.exists():
        raise FileExistsError(f"refusing to reuse v4 model directory: {model_dir}")
    ...
    model_payload = {
        ...
        **deployable,
        "selection": {
            "modelFamily": "fixed 19-input multiframe normalized linear logistic ranker",
        ...
        "dataPolicy": {
            ...
            "confirmationStatus": (
                "retrospective; labels were used by historical v1/v2/v3 research"
            ),
        },
    ...
    dataset_payload = {
        "schemaVersion": 4,
        "kind": "volleycut-side-switch-specialist-development-dataset-v4",
        "createdAt": created_at,
...
def evaluate(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if output_path.exists():
        raise FileExistsError(f"refusing to overwrite v4 evaluation: {output_path}")
    features = _load_features(features_path, args.enforce_source_hash)
    model_payload = _load(model_dir / "model.json")
    if model_payload.get("kind") != MODEL_KIND:
        raise ValueError("model artifact is not side-switch v4")
    deployable = {
        "classifier": model_payload["classifier"],
        "decoder": model_payload["decoder"],
    }
    if model_payload.get("fingerprint") != fingerprint(deployable):
        raise ValueError("v4 model fingerprint is invalid")
    if model_payload.get("sources", {}).get("features", {}).get("sha256") != _sha256(
        features_path
    ):
        raise ValueError("v4 model and feature artifact do not match")
    model = V4Model.from_dict(model_payload["classifier"])
    ...
    payload = {
        "schemaVersion": 4,
        "kind": "volleycut-side-switch-specialist-evaluation-v4",
        ...
        "knownLimitations": {
            ...
            "scoreProxy": (
                "rally gap order remains a noisy point-total proxy; v4 changes only "
                "multi-frame side identity and camera-distance normalization"
            ),
        },
        "predictions": [
            {
