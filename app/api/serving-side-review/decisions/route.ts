import {
  ...
  ServingSideReviewStoreError,
  ServingSideReviewValidationError,
...
function errorResponse(error: unknown): Response {
  if (error instanceof ServingSideReviewValidationError) {
    return Response.json(
      { error: error.message },
      { status: 400, headers: headers() },
    );
  }
  if (error instanceof ServingSideReviewStoreError) {
    return Response.json(
      { error: error.message },
  ...
  return Response.json(
    { error: "The serving-side decisions could not be saved" },
    { status: 503, headers: headers() },
  );
}
...
export async function GET() {
  try {
    return Response.json(await loadServingSideReviewState(), {
      headers: headers(),
    });
  } catch (error) {
...
export async function POST(request: Request) {
  try {
    ...
    return Response.json(await saveServingSideReviewDecisions(body), {
      headers: headers(),
    });
  } catch (error) {
    return errorResponse(error);
