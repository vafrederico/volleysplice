...
function errorResponse(error: unknown): Response {
  if (error instanceof SideSwitchReviewValidationError) {
    return Response.json({ error: error.message }, { status: 400, headers: headers() });
  }
  if (error instanceof SideSwitchReviewStoreError) {
    return Response.json({ error: error.message }, { status: 503, headers: headers() });
  }
  return Response.json(
    { error: "The side-switch decisions could not be saved" },
...
export async function GET() {
  try {
    return Response.json(await loadSideSwitchReviewState(), { headers: headers() });
  } catch (error) {
...
export async function POST(request: Request) {
  try {
    ...
    return Response.json(await saveSideSwitchReviewDecisions(body), {
      headers: headers(),
    });
  } catch (error) {
    return errorResponse(error);
