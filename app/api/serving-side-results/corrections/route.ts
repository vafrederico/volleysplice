import {
  ...
  ServingSideCorrectionStoreError,
  ServingSideCorrectionValidationError,
...
function errorResponse(error: unknown): Response {
  if (error instanceof ServingSideCorrectionValidationError) {
    return Response.json(
      { error: error.message },
      { status: 400, headers: headers() },
    );
  }
  if (error instanceof ServingSideCorrectionStoreError) {
    return Response.json(
      { error: error.message },
  ...
  return Response.json(
    { error: "The serving-side corrections could not be saved" },
    { status: 503, headers: headers() },
  );
}
...
export async function GET() {
  try {
    return Response.json(await loadServingSideCorrectionState(), {
      headers: headers(),
    });
  } catch (error) {
...
export async function POST(request: Request) {
  try {
    ...
    return Response.json(await saveServingSideCorrections(body), {
      headers: headers(),
    });
  } catch (error) {
    return errorResponse(error);
