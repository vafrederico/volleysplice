...
function usePrototype(
  ...
  const [needsSource, setNeedsSource] = useState(review.labMode && review.videoUrl ? false : review.sourceNeedsReconnect);
  const [videoUrl, setVideoUrl] = useState<string | null>(review.videoUrl);
  const [sourceStatus, setSourceStatus] = useState("Saved lab model results and review edits loaded.");
  ...
  const [reviewMessage, setReviewMessageValue] = useState(`${initialClips.filter((clip) => clip.included && !clip.reviewed && clipRequiresConfidenceReview(clip, review.draft.confidenceReviewThreshold)).length} clips remain to check in the current review.`);
  ...
  const [storageMessage, setStorageMessage] = useState("Lab review data loaded");
  ...
  useEffect(() => {
    ...
    setStorageMessage("Saved to this lab configuration on this device");
  }, [review, workingDraft]);

  useEffect(() => {
    setNeedsSource(review.labMode && review.videoUrl ? false : review.sourceNeedsReconnect);
    setVideoUrl(review.videoUrl);
  }, [review.labMode, review.sourceNeedsReconnect, review.videoUrl]);
  ...
  function selectScoreMarker(id: string) {
    ...
    setScoreMessage(`${serveLabel(marker)} selected at ${formatPreciseTime(marker.timestamp)}.`);
  }
  ...
  return {
    ...
    labMode: Boolean(review.labMode), hasSourceFile: Boolean(review.sourceFile), hasProductAnalysis: Boolean(review.productAnalysis), rallyLabel, serveLabel,
    ...
    restoreHistory, historyStorageFailed, resetProjectChanges, applyReviewDraft, playSourceRange,
  ...
  return (
    <div className={className} data-tour="rd-project-selector">
      <label>
        ...
        >
          {!state.labMode && <option value="__new__">＋ Start a new project…</option>}
...
function ReconnectNotice({ state }: { state: Prototype }) {
  if (!state.needsSource || (state.labMode && state.videoUrl)) return null;
  return (
        ...
        {state.clips.map((clip) => (
          ...
          >
            <button
              ...
              aria-label={`${state.rallyLabel(clip.id)}, ${clip.label}, ${clip.included ? "included" : "left out"}`}
            >
              <span>{state.rallyLabel(clip.id)}</span>
    ...
    <section className={`td-clip-inspector ${condensed ? "is-condensed" : ""}`} data-tour={current ? "rd-current-rally" : undefined} aria-labelledby="td-clip-title">
      <header>
        <div>
          ...
          <h2 id="td-clip-title">{state.rallyLabel(clip.id)} <span>{clip.label}</span></h2>
        </div>
        <span className="td-confidence">{state.labMode && clip.id.startsWith("human:") ? "Human export" : clip.origin === "manual" ? "Added by you" : `${Math.round(clip.confidence * 100)}% confidence`}</span>
      </header>
...
function ReviewSummary({
  ...
  return (
    <aside className="td-review-summary" data-review-actions={reviewActions || undefined} data-settings={Boolean(onOpenSettings) || undefined} data-tour={reviewActions ? "rd-review-summary" : undefined}>
      ...
      <div><strong>{clipReviewCount + state.suppressionPending + state.serveReview}</strong><span>{state.labMode ? "serve / clip checks" : "review tasks"}</span></div>
      {onOpenSettings && (
...
function ExportWorkspace({ state, compact = false }: { state: Prototype; compact?: boolean }) {
  const unavailableReason = state.labMode && state.exportKind === "video" && !state.hasSourceFile
  ...
  return (
    <section className={`td-export-panel ${compact ? "is-compact" : ""}`} aria-labelledby="td-export-title">
      <header className="td-section-intro">
        ...
        <p>{state.labMode ? "YouTube chapters use the current review. Download lab edits from the configuration bar to save this experiment." : "Choose one output. Every option is prepared in this browser from the reviewed project."}</p>
      </header>
      <div className="td-export-grid">
        ...
        <div className="td-export-options">
          ...
          <section className="td-readiness" aria-labelledby="td-readiness-title">
            <header><h2 id="td-readiness-title">Ready check</h2><span>{state.labMode ? "Available outputs below" : "Export remains available"}</span></header>
            <ul>
              <li data-ready={!state.needsSource || undefined}><strong>Source video</strong><span>{state.labMode && !state.hasSourceFile ? "NAS video connected for playback" : state.needsSource ? "Reconnect for playback and MP4" : "Connected locally"}</span></li>
          ...
          {state.exportKind === "video" && (
            <>
              ...
              <p className="td-privacy-line">{state.labMode && !state.hasSourceFile ? "This lab plays the NAS source. MP4 rendering is not connected to the lab editor." : "The browser uses the original local video. Nothing is uploaded."}</p>
            </>
          )}
          ...
          <div className="td-export-actions">
            <button className="td-primary-button" type="button" onClick={() => void state.prepareExport()} disabled={Boolean(unavailableReason) || state.exportBusy || (state.exportKind === "video" && state.needsSource)}>{unavailableReason ?? (state.exportKind === "video" && state.needsSource ? "Reconnect source for MP4" : actionLabel)}</button>
            {state.exportKind === "chapters" && (
  ...
  return (
    <div className="td-help-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) state.setHelpOpen(false); }}>
      <aside ref={dialogRef} className="td-help" role="dialog" aria-modal="true" aria-labelledby="td-help-title" aria-describedby="td-help-description" tabIndex={-1}>
        ...
        {!state.labMode && <button type="button" onClick={() => { state.setStage("source"); state.setHelpOpen(false); }}><strong>Set up a new game</strong><span>Choose the game window and camera options.</span></button>}
...
function rallyAgreementLabel(clip: Clip, labMode = false): string {
        ...
        {state.clips.map((clip) => {
          ...
          return (
            <button
              ...
              onClick={() => state.selectClip(clip.id)}
            >
              <span className="rd-register-id">{state.rallyLabel(clip.id)}</span>
              ...
              <small>{rallyAgreementLabel(clip, state.labMode)}{clip.origin === "model" && !clip.id.startsWith("human:") ? ` · ${Math.round(clip.confidence * 100)}%` : ""} · {(clip.end - clip.start).toFixed(1)}s</small>
        ...
        <>
          ...
          {state.selectedScoreMarker && (
            <section className="rd-event-editor" aria-label="Current serve settings">
              <header><strong>{state.serveLabel(state.selectedScoreMarker)}</strong><span>{formatPreciseTime(state.selectedScoreMarker.timestamp)}</span></header>
              <fieldset>
          ...
          {state.workingDraft.cuts.map((cut) => {
            ...
            return (
              <button
                ...
                title={`${state.rallyLabel(cut.id)} · ${RALLY_TIMELINE_LABELS[rallyTimelineStatus(state, cut.id)]} · ${formatPreciseTime(cut.keepStart)}–${formatPreciseTime(cut.keepEnd)}`}
              >
  ...
  useEffect(() => {
    const onKeyDown = (event: KeyboardEvent) => {
      ...
      switch (action) {
        ...
        case "new": if (!state.labMode) state.selectProject(null); break;
...
export function RallyDesk({
  ...
  return (
    <main className="taste-root taste-rally-desk" data-theme={state.dark ? "dark" : "light"}>
      ...
      {state.stage === "review" ? (
        ...
        >
          {(videoResize) => <>
            ...
            {labTools?.({ draft: state.workingDraft, applyDraft: state.applyReviewDraft, seek: state.setPlayhead, playSourceRange: state.playSourceRange, selectedCutId: state.selectedId, currentTime: state.playhead })}
            <ReconnectNotice state={state} />
            ...
            {labComparison?.({ draft: state.workingDraft, applyDraft: state.applyReviewDraft, seek: state.setPlayhead, playSourceRange: state.playSourceRange, selectedCutId: state.selectedId, currentTime: state.playhead })}
            {state.labMode && <div className="rd-mobile-events"><RallyDeskEventRail state={state} /></div>}
          </>}
        </ResizableWorkspace>
      ) : (
      ...
      {state.stage === "review" && !state.labMode && <RallyDeskGuidedTour />}
    </main>
  );
}
              ...
              <aside className="ml-clip-ledger" aria-label="Clip register">
                ...
                {state.clips.map((clip) => (
                  <button key={clip.id} type="button" data-selected={state.selectedId === clip.id || undefined} onClick={() => state.selectClip(clip.id)}>
                    <strong>{state.rallyLabel(clip.id)}</strong><span>{formatTime(clip.start)} to {formatTime(clip.end)}</span><small>{clip.included ? clip.reviewed ? "Included, checked" : "Included, check" : "Left out"}</small>
                  </button>
                ))}
