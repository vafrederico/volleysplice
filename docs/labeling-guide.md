...
5. Use `S` at serve contact and `E` at end of play. Use Shift+`S` to move the closest later rally start and its associated serve marker to the playhead; the editor rejects a move that would overlap another rally, a hard negative, an existing sparse-player observation window, or another serve marker at the target timestamp. The rally strictly containing the playhead is highlighted in both the timeline and rally table. Its number, boundaries, duration, classification, notes, transition cues, and available AI confidence cues also appear in the rally card beside the video, where it can be classified without scrolling to the interval table. The serving-side decision panel beneath **Mark serve contact** shows that rally's current decision; click **Near**, **Far**, or **Review** to change the existing marker or create a missing marker at rally start, and **Remove** to delete it. Clicking any serve-marker dot seeks to its exact timestamp and binds this control to that marker, including when its timestamp sits just outside the rally boundary. During ordinary dead time the card and serving-side control retain the preceding rally, so both can be corrected immediately after its end is marked. Within a rally, `S` moves its start and `E` moves its end to the playhead. In dead time after a rally, `E` extends that immediately preceding rally to the playhead; the editor rejects an extension that would overlap the next rally, a hard negative, or an existing sparse-player observation window. Ignored spans are evaluation masks and may overlay either label type. If a new `S` marker is open and `E` is pressed inside another rally, that rally is split at the playhead: the first part begins at the new `S`, and the remainder keeps the old rally end and metadata. Sparse tracklets are repartitioned by their valid boundary windows. This intentionally leaves the remainder selected for correction; move its start to the next real serve or delete it if it is only the old AI tail. Exactly touching rallies cannot be completed because the live/dead model needs positive dead time between distinct events.
6. Shift-click two or more consecutive editable rally bars or rally-table rows, then choose **Merge selected**. The merged rally spans from the earliest selected start through the latest selected end, preserving the first rally's start metadata and the last rally's end metadata. Its tags and notes are combined. Only the earliest serve marker associated with the selected rallies is kept; later internal serve markers are removed. If the retained marker came from a later selected rally because the earliest selected rally had none, it is moved to the merged rally start. A merge is blocked when it would consume a hard negative or invalidate sparse player tracks at an internal boundary.
7. Delete the highlighted rally with **Delete selected**, `Delete`, or `Backspace`. Keyboard deletion is disabled while focus is in a form control.
8. If teams switch court sides in this video or format, press `X` at the switch. Side switches are point markers rather than rally intervals; their time and optional note can be edited or deleted in the **Side switches** section.
