...
For advanced events, the 2025 VNL-STES project publishes BSD-3-Clause model code and links a resized archive described as 6,137 annotations across 1,028 rallies for serves, receives, sets, spikes, blocks, and scores. Its serve spotting was substantially stronger than score-event spotting; score events achieved only 43.05 mTAP and were affected by occlusion and confusion with failed receives. The archive has no explicit dataset license, no trained weights were found, and its observed counts do not fully match the paper, so it requires a rights and schema review before reuse. See the [deeper artifact audit](./volleyball-cutting-vnl-reuse-decision.md), [VNL-STES project](https://hoangqnguyen.github.io/stes/), and [code](https://github.com/hoangqnguyen/spot).
