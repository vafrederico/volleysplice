...
final class NanoProfiler {
    ...
    synchronized void add(String name, long elapsedNanos) {
        nanos.merge(name, Math.max(0, elapsedNanos), Long::sum);
    }

    synchronized Map<String, Double> milliseconds() {
    ...
    synchronized void appendMilliseconds(String prefix, Map<String, Double> source) {
